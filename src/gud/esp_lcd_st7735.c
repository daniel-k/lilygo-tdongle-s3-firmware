 ...
 * (commit fda1b85). Only the include path adjusted. See
 * licenses/T-Dongle-S3-LICENSE for the upstream MIT terms.
 *
 * Compiled in only on T-Dongle S3 builds — plain ESP32-S3 boards have
 * no LCD wired and don't include any GUD function.
 */
...
static esp_err_t panel_st7735_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    if (on_off) command = LCD_CMD_DISPON;
    else        command = LCD_CMD_DISPOFF;
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, command, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}
